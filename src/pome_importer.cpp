...
PomeValue Importer::import(const std::string& logicalPath) {
    std::cerr << "DEBUG: Importer::import(" << logicalPath << ") called." << std::endl;
    if (isCached(logicalPath)) {
        std::cerr << "DEBUG: " << logicalPath << " is cached." << std::endl;
        return moduleCache_[logicalPath];
    }

    ScopedLoadingModule loader(loadingModules_, logicalPath); // Cyclic import detection

    ModuleCandidate candidate = findModuleCandidate(logicalPath);
    std::cerr << "DEBUG: Candidate type for " << logicalPath << ": " << static_cast<int>(candidate.type) << std::endl;
    ...
    PomeModule* moduleObj = interpreter_.getGC().allocate<PomeModule>(); // Allocate a PomeModule for exports
    RootGuard moduleObjGuard(interpreter_.getGC(), moduleObj); // Protect moduleObj immediately

    std::cerr << "DEBUG: moduleObj pointer address before PomeValue construction: " << (void*)moduleObj << std::endl;
    ...
    if (candidate.type == ModuleCandidateType::POME_SCRIPT_FILE || candidate.type == ModuleCandidateType::POME_PACKAGE_DIR) {
        std::cerr << "DEBUG: Loading Pome script/package file: " << candidate.path << std::endl;
        ...
        if (!program) { // Defensive check
            throw std::runtime_error("Parsing failed for module: " + filePath);
        }
        std::cerr << "DEBUG: Program parsed for " << logicalPath << std::endl;
        ...
        RootGuard prevEnvGuard(interpreter_.getGC(), previousEnvironment); // Root the previous environment
        Environment* moduleEnv = interpreter_.getGC().allocate<Environment>(interpreter_.globalEnvironment_); // Parent is global
        std::cerr << "DEBUG: Environment created for " << logicalPath << std::endl;
        
        interpreter_.currentEnvironment_ = moduleEnv; // Set module's environment
        interpreter_.exportStack_.push_back(moduleObj); // Push module for exports
        std::cerr << "DEBUG: Environment/export stack pushed for " << logicalPath << std::endl;

        try {
            std::cerr << "DEBUG: Executing program for " << logicalPath << std::endl;
            moduleObj->ast_root->accept(interpreter_); // Execute the module's program
            std::cerr << "DEBUG: Program executed for " << logicalPath << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "DEBUG: Exception during execution of " << logicalPath << ": " << e.what() << std::endl;
        ...
        std::cerr << "DEBUG: Environment/export stack popped for " << logicalPath << std::endl;

    } else if (candidate.type == ModuleCandidateType::NATIVE_MODULE_FILE) {
        std::cerr << "DEBUG: Loading native module file: " << candidate.path << std::endl;
        // Load the native module using the Interpreter's loadNativeModule
        PomeValue nativeResult = interpreter_.loadNativeModule(candidate.path);
        if (nativeResult.isTable()) { // Expect native module to return a Table of exports
            std::cerr << "DEBUG: Native module returned a Table for " << logicalPath << std::endl;
            // Copy exports from the returned table to our allocated PomeModule
        ...
        } else if (!nativeResult.isNil()) { // Native module returned something unexpected
            throw std::runtime_error("Native module '" + logicalPath + "' returned a non-Table, non-Nil value from PomeInitModule.");
        }
        std::cerr << "DEBUG: Native module processing complete for " << logicalPath << std::endl;
    } else {
    ...
    std::cerr << "DEBUG: Attempting to cache module " << logicalPath << ". moduleValue type: " << moduleValue.toString() << std::endl;
    moduleCache_[logicalPath] = moduleValue;
    std::cerr << "DEBUG: Module cached for " << logicalPath << std::endl;
    return moduleValue;
}
        ...
        if (FileUtils::exists(pkgJsonPath)) {
            try {
                ...
                if (it != pkgInfo.nativeModules.end()) {
                    std::string nativeLibPath = potentialPkgRoot + "/lib/" + "lib" + moduleName + getNativeExtensionSuffix();
                ...
                std::cerr << "Warning: Malformed pome_pkg.json in " << potentialPkgRoot << ": " << e.what() << std::endl;
            }
        }
    }
